...
def abstract2sents(abstract):
  ...
  while True:
    try:
      #print(type(config.SENTENCE_START), type(cur))
      start_p = abstract.index(config.SENTENCE_START.encode(), cur)
      end_p = abstract.index(config.SENTENCE_END.encode(), start_p + 1)
