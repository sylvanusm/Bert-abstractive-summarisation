...
class Summarizer(nn.Module):
    def __init__(self, is_draft, toeknizer, pretrained_model="bert-base-uncased", model_file_path=None, is_eval=False, load_optim=False):
        super(Summarizer, self).__init__()
        self.is_draft = is_draft
        self.toeknizer = toeknizer
        if is_draft: self.encoder = BertModel.from_pretrained(config.model_name)
        else: BertForMaskedLM.from_pretrained(config.model_name)
